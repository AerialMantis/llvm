...
* A memory pool has a *reserved size high* (in bytes) which tracks a watermark
  value, which is the highest the reserve size has been since the memory pool
  was created or the value was last reset.
* A memory pool has a *used size high* (in bytes) which tracks a watermark
  value, which is the highest the used size has been since the memory pool
  was created or the value was last reset.
...
_Returns_: The current reserved memory for the pool.
...
_Returns_: The watermark value for the highest the reserved memory has been
since the memory pool was created or since it was last reset, at the point the
function is called.
...
_Returns_: The current used memory for the pool.
...
_Returns_: The watermark value for the highest the used memory has been since
the memory pool was created or since it was last reset, at the point the
function is called.
...
_Effects_: Resets the watermark value for the highest the reserved memory has
been to the current reserved size.
...
_Effects_: Resets the watermark value for the highest the used memory has
been to the current used size.
